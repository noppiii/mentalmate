...
Dessert chocolate cake lemon drops jujubes. Biscuit cupcake ice cream bear claw brownie brownie marshmallow.</textarea
                            ...
                            <ul class="list-unstyled chat-contact-list" id="chat-list">
                                ...
                                @foreach ($mahasiswa as $data)
                                    <li class="chat-contact-list-item">
                                        <a href="{{ route('psikolog.konsultasi.index', ['receiverId' => $data->id, 'receiverType' => 'MahasiswaModel']) }}"
                                           class="d-flex align-items-center">
                                            <div class="flex-shrink-0 avatar avatar-online">
                                                <img
                                                    src="{{ asset('store/user/photo/mahasiswa/' . $data->profile_photo_path) }}"
                    ...
                    <div class="col app-chat-history bg-body">
                        <div class="chat-history-wrapper">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="d-flex overflow-hidden align-items-center">
                                        ...
                                        <div class="flex-shrink-0 avatar">
                                            <img src="{{ asset('store/user/photo/mahasiswa/' . $detailMahasiswa->profile_photo_path) }}" alt="Avatar" class="rounded-circle" data-bs-toggle="sidebar" data-overlay data-target="#app-chat-sidebar-right" />
                                        </div>
                                        <div class="chat-contact-info flex-grow-1 ms-2">
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="dropdown">
                                            <i class="ti ti-dots-vertical cursor-pointer" id="chat-header-actions" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></i>
                            ...
                            <div class="chat-history-body bg-body">
                                <ul class="list-unstyled chat-history" id="chat-history">
                                    @foreach ($messages as $message)
                                        <li class="chat-message {{ $message->sender_id == Auth::guard('psikolog')->user()->id && $message->sender_type == 'Psikolog' ? 'chat-message-right' : '' }}">
                                            <div class="d-flex overflow-hidden">
                                                @if ($message->sender_id == Auth::guard('psikolog')->user()->id && $message->sender_type == 'Psikolog')
                            ...
                            <div class="chat-history-footer shadow-sm">
                                <form action="{{ route('psikolog.konsultasi.store') }}" method="POST"
                                      class="form-send-message d-flex justify-content-between align-items-center">
                                    @csrf
                                    <input type="hidden" name="sender_type"
                                           value="{{ auth()->guard('psikolog')->check() ? 'Psikolog' : 'Mahasiswa' }}">
                                    <input type="hidden" name="sender_id"
                                           value="{{ auth()->guard('psikolog')->check() ? auth()->guard('psikolog')->user()->id : auth()->guard('mahasiswa')->user()->id }}">
                                    <input type="hidden" name="receiver_type"
                                           value="{{ $receiverType ?? 'Mahasiswa' }}">
        ...
        <script>
            function fetchMessages() {
                $.ajax({
                    url: '{{ route("psikolog.getMessages", ["receiverId" => $receiverId, "receiverType" => $receiverType]) }}',
                    method: 'GET',
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                document.querySelector('.form-send-message').addEventListener('submit', function(event) {
                    ...
                    axios.post('/psikolog/my-konsultasi/store', formData)
                        .then(response => {
                            console.log('Pesan yang baru dikirim:', response.data);
