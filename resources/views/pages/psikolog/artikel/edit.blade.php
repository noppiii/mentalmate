...
@section('title')
    Edit Artikel | {{ config('app.name') }}
@endsection
@section('pages')
    Edit Artikel Psikolog
@endsection
@section('content')
        ...
        <div class="col-lg-12 mb-4 order-0">
              ...
              <div class="d-flex align-items-end row">
                <div class="col-sm-7">
                  <div class="card-body">
                    ...
                    <p>Di sini anda dapat mengubah artikel</p>
         ...
         <div class="card mb-4">
            <form class="card-body" action="{{ route('my-artikel.update', $artikel->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    <div class="row mb-3 mt-4">
                        <label class="col-sm-2 form-label" for="basic-icon-default-email">Thumbnail Artikel <small class="text-danger">*Jika ingin mengubah thumbnail</small></label>
                        <div class="col-sm-10">
                            @if ($artikel->tumbnail)
                            <div class="avatar avatar-xl mt-2 mb-2">
                                <img src="{{ asset('store/artikel/thumbnail/' . $artikel->tumbnail) }}" alt="Avatar" class="rounded-circle" />
                              </div>
                             @else
                                 <p>Thumbnail Artikel Tidak Ditemukan</p>
                             @endif
                            ...
                            <div class="select2-primary">
                                <select id="select2Primary" class="select2 form-select" name="kategori_artikel[]" multiple>
                                    @foreach ($allCategory as $data)
                                        <option value="{{ $data->id }}" {{ $artikel->kategoriArtikels->contains($data->id) ? 'selected' : '' }}>
                                            {{ $data->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="select2-info">
                                <select id="select2Info" class="select2 form-select" name="tag_artikel[]" multiple>
                                    @foreach ($allTag as $data)
                                        <option value="{{ $data->id }}" {{ $artikel->tagArtikels->contains($data->id) ? 'selected' : '' }}>
                                            {{ $data->nama }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="row g-3">
                    <div class="row mb-3 mt-4">
                        ...
                        <div class="col-sm-10">
                            <div id="full-editor">{!! $artikel->content !!}</div>
                            <textarea id="content" name="content" style="display:none;">{!! $artikel->content !!}</textarea>
                        </div>
                    </div>
                </div>
                <div class="pt-4">
                    <button type="submit" class="btn btn-primary me-sm-3 me-1">Update</button>
                    <a href="{{ route('artikel.index') }}" class="btn btn-label-secondary">Cancel</a>
                </div>
            </form>
          </div>
    </div>
