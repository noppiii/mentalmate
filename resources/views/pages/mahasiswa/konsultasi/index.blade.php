                            ...
                            <ul class="list-unstyled chat-contact-list" id="chat-list">
                                ...
                                @foreach ($allPsikolog as $data)
                                    <li class="chat-contact-list-item {{ $receiverId == $data->id ? 'active' : '' }}">
                                        <a href="{{ route('mahasiswa.konsultasi.index', ['receiverId' => $data->id, 'receiverType' => 'PsikologModel']) }}"
                                           class="d-flex align-items-center">
                                            <div class="flex-shrink-0 avatar avatar-online">
                                                <img
                                                    src="{{ asset('store/user/photo/psikolog/' . $data->profile_photo_path) }}"
                                            ...
                                            <li class="chat-message chat-message">
                                                <div class="d-flex overflow-hidden">
                                                    <div class="user-avatar flex-shrink-0 me-3">
                                                        <div class="avatar avatar-sm">
                                                            <img src="{{ asset('store/user/photo/psikolog/' . $detailPsikolog->profile_photo_path) }}"
                                                                 alt="Avatar" class="rounded-circle"/>
                                                        </div>
                                                    </div>
                                                    <div class="chat-message-wrapper flex-grow-1">
                            ...
                            <div class="chat-history-footer shadow-sm">
                                <form action="{{ route('mahasiswa.konsultasi.store') }}" method="POST"
                                    ...
                                    <input type="hidden" name="sender_type"
                                           value="{{ auth()->guard('mahasiswa')->check() ? 'Mahasiswa' : 'Psikolog' }}">
                                    <input type="hidden" name="sender_id"
                                           value="{{ auth()->guard('mahasiswa')->check() ? auth()->guard('mahasiswa')->user()->id : auth()->guard('psikolog')->user()->id }}">
